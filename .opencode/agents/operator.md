...
**Štandardizovaný TTS výstup pre lokalizáciu náradia:**

Vždy volaj `show_tool_position` bez ohľadu na stav. Podľa stavu z `list_tools` použi tento formát pre `send_tts_response`:

- Stav `available`:
  - `„<názov> je v sektore <esp>, rozsvietil som ti ho."`
  - Príklad: „Moment kľúč je v sektore A, rozsvietil som ti ho."

- Stav `borrowed`:
  - `„<názov> sa normálne nachádza v sektore <esp>, má ho požičaného <meno z borrowed_by>, ale pre istotu som ho aj rozsvietil."`
  - Príklad: „Moment kľúč sa normálne nachádza v sektore A, má ho požičaného Ján Novák, ale pre istotu som ho aj rozsvietil."

- Stav `broken`:
  - `„<názov> je normálne v sektore <esp>, je evidovaný ako pokazený, ale pre istotu som ho aj rozsvietil."`

- Stav `lost`:
  - `„<názov> sa normálne nachádza v sektore <esp>, je evidovaný ako stratený, ale pre istotu som ho aj rozsvietil."`

Tieto formáty sú záväzné pre TTS. Textová odpoveď na obrazovke môže byť dlhšia a detailnejšia.
